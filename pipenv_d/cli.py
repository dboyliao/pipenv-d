...
def cli():
    try:
        ...
        if lockfile_path.exists():
            with active_lockfile.open(
                "w", encoding="utf8"
            ) as out_fid, lockfile_path.open("r", encoding="utf8") as in_fid:
                out_fid.write(in_fid.read())
        ret_code = _run_pipenv()
        if active_lockfile.exists():
            with lockfile_path.open(
                "w", encoding="utf8"
            ) as out_fid, active_lockfile.open("r", encoding="utf8") as in_fid:
                out_fid.write(in_fid.read())
